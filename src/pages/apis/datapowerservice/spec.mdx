...
|`dataPowerMonitor`   |DataPower Monitor configuration                                  |No      |See doc below|
...
<AnchorLinks small>
  ...
  <AnchorLink>dataPowerMonitor</AnchorLink>
...
The `debug` property is a user toggleable debugging utility. It sets an environment variable `DPOPERATOR_DEBUG` in each pod which makes all DataPower provided init scripts output debug level information. If not set, it will default to `false`.
...
Defaults to `ibmcom/datapower-monitor:latest`.
...
spec:
  datapowerMonitor:
    image: ibmcom/datapower-monitor:latest
...
The `Storage` property is a list of storage definitions. Storage definitions fall into two categories: ephermeral and persistent. The following fields can be modified:
...
`size` is the size of the persisted volume. The expected format is some form of `#Gi`. For more information on resource definitions, [see Resources doc](https://github.com/kubernetes/community/blob/master/contributors/design-proposals/scheduling/resources.md).

This field applies to both ephermeral and persistent types.

#### `class`

`class` is a direct pass through to the `storageClassName` field for the Persistent Volume Claim. It determines which class to use for the volume. [StorageClass doc](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#class-1)

This field applies only to the persistent type.

#### `volumeMode`

`volumeMode` is a switch between `Filesystem` and `Block` volume types. Our default type is `Filesystem`. [VolumeMode doc](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#volume-modes)
