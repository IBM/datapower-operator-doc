...
The table below shows the **supported** channels for this CRD version (`v1beta3`) and operator version (`1.3.x`). Supported channels indicate channels which can be specified by `spec.version` for which this version of the DataPower Operator will understand and reconcile.

|Channel   |Stream    |Latest        |
|----------|----------|--------------|
|`10.0-lts`|`10.0.1.x`|`10.0.1.2`    |
|`10.0-cd` |`10.0.x.y`|`10.0.2.0`    |

#### Available

The table below shows the **available** channels for this CRD version (`v1beta3`) and operator version (`1.3.x`). Available channels (and version) are advertised in Cloud Pak for Integration as targets for operand upgrade. To enable and support the Cloud Pak for Integration EUS strategy, version `1.2.x` will only advertise EUS operand channels and versions.
...
`lifecycle` allows scripts to be executed after starting and before stopping DataPower container. For more details on container lifecycle hooks, please refer to [Kubernetes documentation](https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/).
