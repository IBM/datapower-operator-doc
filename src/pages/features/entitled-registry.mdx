...
If the operator detects an `ibm-entitlement-key` Secret in the same namespace as the `DataPowerService` Custom Resource, the operator will automatically use this Secret to pull the IBM DataPower images. You do not need to manually add this secret to your `DataPowerService` CR spec.
